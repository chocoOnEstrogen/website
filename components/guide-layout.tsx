...
function TocItems({ items, level = 0, activeId }: { items: TocItem[]; level?: number; activeId?: string }) {
      ...
      {items.map((item) => (
        <li key={item.id}>
          ...
          {expanded[item.id] && item.children.length > 0 && (
            <TocItems items={item.children} level={level + 1} activeId={activeId} />
          )}
        </li>
      ))}
    </ul>
...
function HeaderLink({ id, children }: { id: string; children: React.ReactNode }) {
          ...
          <aside className="lg:w-64 lg:flex-shrink-0">
            <nav className="sticky top-24 space-y-6">
              {Object.entries(groupedGuides).map(([category, categoryGuides]) => (
                <div key={category}>
                  <h3 className="mb-3 font-semibold text-purple-400">{category}</h3>
              ...
              {currentGuide && currentGuide.metadata.toc && (
                <div className="xl:hidden">
                  <h3 className="mb-3 font-semibold text-purple-400">On this page</h3>
            ...
            {currentGuide && (
              <>
                <header className="mb-8">
                  <h1 className="mb-4 text-4xl font-bold">{currentGuide.metadata.title}</h1>
                  <div className="flex flex-wrap items-center gap-4 text-sm text-gray-400">
                    {currentGuide.metadata.date && (
                      <time dateTime={currentGuide.metadata.date}>
                        {new Date(currentGuide.metadata.date).toLocaleDateString('en-US', {
                          year: 'numeric',
                          month: 'long',
                          day: 'numeric'
                        })}
                  ...
                  {currentGuide.metadata.description && (
                    <p className="mt-4 text-lg text-gray-400">{currentGuide.metadata.description}</p>
                  )}
                </header>
                ...
                <nav className="mt-16 flex justify-between border-t border-gray-800 pt-8">
                  {currentGuide.metadata.prev && (
                    ...
                    >
                      <svg className="h-5 w-5 transition-transform group-hover:-translate-x-1" viewBox="0 0 24 24" fill="none" stroke="currentColor">
